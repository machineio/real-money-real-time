fun.views.campaigns = Backbone.View.extend({
    ...
    */
    updateInboundDetails: function(event){
        ...
        this.status = $('input[name="inbound_campaign_status"]:checked');
        this.NuStatus = this.$('#inbound-campaign-status');
        ...
        nuStatus = this.NuStatus.text();

        console.log(status, nuStatus);
        
        campaignUuid = this.uuid.text();
        accountStuff = this.account.text();

        console.log(campaignUuid, accountStuff);

        update = new fun.models.InboundCampaign({'uuid':campaignUuid});
